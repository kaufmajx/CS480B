    ...
    implements ActionListener, ListSelectionListener, StreetSegmentSubject
{
  ...
  public GeocodeDialog(final JFrame owner, final Geocoder geocoder)
  {
    ...
    populate(categoryField,
        new String[] {"Aly", "Arc", "Ave", "Blvd", "Br", "Brg", "Byp", "Cir", "Cres", "Cswy", "Ct",
            "Ctr", "Cv", "Dr", "Expy", "Fwy", "Grd", "Hwy", "Ln", "Loop", "Mal", "Pass", "Path",
            "Pike", "Pky", "Pl", "Plz", "Ramp", "Rd", "Row", "Rte", "Rue", "Run", "Spur", "Sq",
            "St", "Ter", "Tpke", "Trce", "Trl", "Tunl", "Walk", "Way", "Xing"});
  ...
  public void actionPerformed(final ActionEvent evt)
  {
    ...
    try
    {
      ...
      for (double[] location : locations)
        lonlat.add(location[0] + "," + location[1]);
    ...
    catch (NumberFormatException nfe)
    {
      JOptionPane.showMessageDialog(this,
          QUOTE + numberField.getText() + QUOTE + " is not a valid street number!", "Input Error",
   ...
   * 
   * @param items
   *          The items to include (after the blank)
  ...
  /**
   * Notify all of the StreetSegmentObserver objects of a handleStreetSegment() message.
   * 
   * @param segmendIDs
  ...
  private void notifyStreetSegmentObservers(final List<String> segmendIDs)
  {
    for (StreetSegmentObserver observer : observers)
      observer.handleStreetSegments(segmendIDs);
  }
   ...
   * 
   * @param comboBox
   *          The JComboBox to populate
   * @param items
   *          The items to populate it with (or null to reset)
   */
